    ...
    <div class="lunbo">
        ...
        <div class="lunbo_wrapper">
            ...
            <el-table v-if="tableData.length > 0" :data='tableData' border 
                ...
                <!-- 添加日期 -->
                <el-table-column prop="type" label="创建时间" align="center" width="130">
                ...
                <el-table-column prop="image" label="美食图片" align="center" width="100">
                    <template slot-scope="scope">                        
                        <img style="width: 40px; height: 40px" :src="scope.row.pic" alt="123">
                    </template>
                </el-table-column>

                <el-table-column prop="describer" label="美食名称" align="center" width="150">
                    <template slot-scope="scope">
                        <span style="margin-left: 10px">{{ scope.row.foodName }}</span>
                    </template>
                </el-table-column>

                <el-table-column prop="describer" label="所属分类编号" align="center" width="110">
                    <template slot-scope="scope">
                        <span style="margin-left: 10px">{{ scope.row.ClassifyNo }}</span>
                    </template>
                </el-table-column>

                <el-table-column prop="describer" label="编号名称" align="center" width="120">
                    <template slot-scope="scope">
                        <span style="margin-left: 10px">{{ scope.row.ClassifyName }}</span>
                    </template>
                </el-table-column>

                <el-table-column prop="describer" label="美食描述" align="center" width="313">
                    <template slot-scope="scope">
...
<script>
    ...
    import { getClassifyDataBase ,adminDelClassify} from "../../../http/index"
    export default {
        name:"lunbo",
        data(){
            return {
        ...
        methods:{
            handleEdit(index, row) {
                ...
                localStorage.setItem("add_classify_item",row._id)
                // 点击编辑按钮之后页面跳转到编辑页面
                this.$router.push("/index/editclassify")
            },
            ...
            handleDelete(index, row) {
                ...
                Dialog.confirm({
                    ...
                    let result = adminDelClassify(id)
                    result.then((data)=>{
                        if(data.errCode === 0){
                            this.getClassifyData() //重新请求数据
                        }
                    })
                    .catch((error)=>{
                        console.log(error)
            ...
            addItem(){
                // 点击页面增加按钮，跳转到添加项目页面
                this.$router.push("/index/addclassify")
            }, 

            async getClassifyData(){
                // 获取数据库数据展示页面                
                let result = await getClassifyDataBase()              
                if(result.errCode === 0){
                    // 查询返回数据成功
                    this.alltableData = result.dataArr
                    console.log("PPPPPPP = ",this.alltableData)
                    // 设置分页数据
                    this.setPaginations()
                }else{
...
.lunbo .lunbo_wrapper .add_btn .add_btn_pri{
    position: absolute;
    right: 0px;
}
