<template>
    <div class="edit_lunbo_item">        
        <div class="editlbitem_content">
            <div class="panel_heading">编辑精品美食推荐项目</div>
...
<script>
    ...
    import {getJPFoodRecInfo,editJPFoodRec} from "../../../http/index"
    export default {
        ...
        methods:{
            ...
            async editFoodRecItem(){
                ...
                let _id = localStorage.getItem("add_jpfoodrec_item")
                let data = new FormData()
                data.append("foodName",foodName)
                data.append("desc",description)
                data.append("pic",this.pic)
                data.append("_id",_id)
                let result = await editJPFoodRec(data)
                if(result.errCode === 0){
                    // 数据返回成功
                    // 页面跳转
                    this.$router.push("/info/jpFoodRec")
                }else{
                    // 返回失败结果
            ...
            async getFoodRecItemInfo(){
                // add_lb_items
                // 获取轮播图的编辑信息,去数据中查询
                let id = localStorage.getItem("add_jpfoodrec_item")
                // console.log("&&&&&&&********** = ",id)
                let result = await getJPFoodRecInfo(id)
                if(result.errCode === 0){
