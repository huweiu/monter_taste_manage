...
<script>
    ...
    import { getJCFoodDataBase ,adminDelJCFood} from "../../../http/index"
    export default {
        name:"lunbo",
        data(){
            return {
        ...
        methods:{
            handleEdit(index, row) {
                ...
                localStorage.setItem("add_jcfood_item",row._id)
                // 点击编辑按钮之后页面跳转到编辑页面
                this.$router.push("/info/editjcfood")
            },
            ...
            handleDelete(index, row) {
                ...
                Dialog.confirm({
                    ...
                    let result = adminDelJCFood(id)
                    result.then((data)=>{
                        if(data.errCode === 0){
                            this.getJCFoodData() //重新请求数据
                        }
                    })
                    .catch((error)=>{
                        console.log(error)
            ...
            addItem(){
                // 点击页面增加按钮，跳转到添加项目页面
                this.$router.push("/info/addjcfood")
            },
            // \\ACER-PC/upload/lunbo/2ql_rZXBiDG_2IJrkqWmgPFF.jpg
            async getJCFoodData(){
                // 获取数据库数据展示页面                
                let result = await getJCFoodDataBase()              
                if(result.errCode === 0){
                    // 查询返回数据成功
