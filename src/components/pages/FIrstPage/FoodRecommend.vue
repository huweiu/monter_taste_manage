...
<script>
    ...
    import { getFoodRecDataBase ,adminDelFoodRec} from "../../../http/index"
    export default {
        name:"lunbo",
        data(){
            return {
        ...
        methods:{
            handleEdit(index, row) {
                ...
                localStorage.setItem("add_foodrec_item",row._id)
                // 点击编辑按钮之后页面跳转到编辑页面
                this.$router.push("/info/editFoodRec")
            },
            ...
            handleDelete(index, row) {
                ...
                Dialog.confirm({
                    ...
                    let result = adminDelFoodRec(id)
                    result.then((data)=>{
                        if(data.errCode === 0){
                            this.getFoodRecData() //重新请求数据
                        }
                    })
                    .catch((error)=>{
                        console.log(error)
            ...
            addItem(){
                // 点击页面增加按钮，跳转到添加项目页面
                this.$router.push("/info/addfoodrec")
            },
            // \\ACER-PC/upload/lunbo/2ql_rZXBiDG_2IJrkqWmgPFF.jpg
            async getFoodRecData(){
                // 获取数据库数据展示页面                
                let result = await getFoodRecDataBase()              
                if(result.errCode === 0){
                    // 查询返回数据成功
