    ...
    <div class="lunbo">
        ...
        <div class="lunbo_wrapper">
            ...
            <el-table v-if="tableData.length > 0" :data='tableData' border 
                ...
                <el-table-column prop="image" label="美食图片" align="center" width="180">
                    <template slot-scope="scope">                        
                ...
                <el-table-column prop="describer" label="美食描述" align="center" width="333">
                    <template slot-scope="scope">
...
<script>
    ...
    import { getJPFoodRecDataBase ,adminDelJPFoodRec} from "../../../http/index"
    export default {
        name:"lunbo",
        data(){
            return {
        ...
        methods:{
            handleEdit(index, row) {
                ...
                localStorage.setItem("add_jpfoodrec_item",row._id)
                // 点击编辑按钮之后页面跳转到编辑页面
                this.$router.push("/info/editjpFoodRec")
            },
            ...
            handleDelete(index, row) {
                ...
                Dialog.confirm({
                    ...
                    let result = adminDelJPFoodRec(id)
                    result.then((data)=>{
                        if(data.errCode === 0){
                            this.getJPFoodRecData() //重新请求数据
                        }
                    })
                    .catch((error)=>{
                        console.log(error)
            ...
            addItem(){
                // 点击页面增加按钮，跳转到添加项目页面
                this.$router.push("/info/addjpFoodRec")
            },
            // \\ACER-PC/upload/lunbo/2ql_rZXBiDG_2IJrkqWmgPFF.jpg
            async getJPFoodRecData(){
                // 获取数据库数据展示页面                
                let result = await getJPFoodRecDataBase()              
                if(result.errCode === 0){
                    // 查询返回数据成功
